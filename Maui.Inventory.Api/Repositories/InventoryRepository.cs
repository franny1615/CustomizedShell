...
FETCH NEXT {request.ItemsPerPage} ROWS ONLY";
    ...
    #region GET INVENTORY HISTORY
    public async Task<APIResponse<PaginatedQueryResponse<Models.Inventory>>> GetHistory(
        PaginatedRequest request, 
        int adminId, 
        int inventoryId)
    {
...
(
    ...
    #region UPDATE
    public async Task<APIResponse<bool>> Update(Models.Inventory inventory, Models.Inventory? previousInventory)
    {
        #region HISTORY QUERY
        if (previousInventory != null)
        {
            bool descriptionChanged = inventory.Description != previousInventory.Description;
            bool statusChanged = inventory.Status != previousInventory.Status;
            bool quantityChanged = inventory.Quantity != previousInventory.Quantity;
            bool quantityTypeChanged = inventory.QuantityType != previousInventory.QuantityType;
            bool locationChanged = inventory.Location != previousInventory.Location;

            bool shouldInsert = descriptionChanged || statusChanged || quantityChanged || quantityTypeChanged || locationChanged;
            if (shouldInsert)
            {
                string historyQuery = $@"
...
VALUES
(
    {previousInventory.Id},
    {previousInventory.AdminId},
    '{previousInventory.Description}',
    '{previousInventory.Status}',
    {previousInventory.Quantity},
    '{previousInventory.Barcode}',
    '{previousInventory.Location}',
    '{previousInventory.QuantityType}',
    GETDATE(),
    '{previousInventory.CreatedOn}'
)";
                try
                {
                    await SQLUtils.QueryAsync<object>(historyQuery);
                }
                catch { }
            }
        }
...
WHERE AdminId = {inventory.AdminId}
AND InvId = {inventory.Id};
